# 输入文件夹路径： 程序遍历四个文件夹（Landsat5_COMID、Landsat7_COMID、HLSL30_COMID、HLSS30_COMID）中的文件。
#
# 查找相同文件名： 检查四个文件夹中共有的文件名。
#
# 合并数据：
#
# 读取相同文件名的所有文件。
# 将文件中的 date 列数据合并。
# 使用 pandas 对日期进行排序，并确保数据完整性。
# 保存结果： 将排序后的结果保存到目标文件夹 Combined_COMID，文件名保持不变，格式为 .xlsx。
#
# 执行结果： 程序执行完毕后，所有合并文件将存储在目标文件夹中。
...
for file_name in file_names:
    ...
    if combined_data:
        ...
        combined_df['date'] = pd.to_datetime(combined_df['date'], errors='coerce')  # 确保日期格式正确
        combined_df = combined_df.dropna(subset=['date'])  # 删除无效日期
