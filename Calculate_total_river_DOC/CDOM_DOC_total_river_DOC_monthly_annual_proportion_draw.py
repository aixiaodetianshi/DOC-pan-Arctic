# 季节变化比例  # English: Seasonal variation ratio
...
file_path = r"D:\UZH\2024\20240122 Nutrient and Organic Carbon references\DOC\DOC_update_20250203\Total_DOC_monthly\All_month\Total_monthly_DOC_All_Parts_Proportion_10.csv"
output_path = r"D:\UZH\2024\20240122 Nutrient and Organic Carbon references\DOC\DOC_update_20250203\Total_DOC_monthly\All_month\Total_monthly_DOC_All_Parts_Proportion_10.tif"
...
years = data['year']
Total_DOC_Proportion = data['Total_DOC_Proportion']

# 计算平均值和不确定性  # English: Calculate mean and uncertainty
mean_Total_DOC_Proportion = np.mean(Total_DOC_Proportion)

# 计算趋势线  # English: Calculate trend lines
slope, intercept, r_value, p_value, std_err = linregress(years, Total_DOC_Proportion)
trend_label = f'Trend={slope:.3f}'

# 创建图像  # English: Create an image
fig, ax = plt.subplots(figsize=(8.5/2.54, 3.5/2.54))  # 转换为厘米  # English: Convert to centimeters

# 绘制季节占比变化曲线  # English: Draw the seasonal proportion change curve
ax.plot(years, Total_DOC_Proportion, color='royalblue', linewidth=1.5, label='Monthly Proportion')

# 添加趋势线 (回归)  # English: Add a trend line
sns.regplot(x=years, y=Total_DOC_Proportion, scatter=False, color='darkblue',
...
ax.axhline(mean_Total_DOC_Proportion, color='red', linestyle='-.', linewidth=1, label=f'Average={mean_Total_DOC_Proportion:.3f}')

# 图例与标签  # English: Legends and labels
ax.set_xlabel('Year', fontsize=6, fontname='Arial')
ax.set_ylabel('Monthly proportion (%)', fontsize=6, fontname='Arial')
