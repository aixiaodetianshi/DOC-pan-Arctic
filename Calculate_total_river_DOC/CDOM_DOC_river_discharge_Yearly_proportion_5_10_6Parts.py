# 计算得到每一parts 1984-2018年期间river discharge年总量  # English: Calculate each
...
# 遍历文件夹内所有 xlsx 文件  # English: Traverse all in the folder
for file_name in os.listdir(input_folder):
    if file_name.endswith(".xlsx"):
        file_path = os.path.join(input_folder, file_name)

        # 读取 xlsx 文件，仅选择所需列  # English: Read
        df = pd.read_excel(file_path, usecols=['year', 'Annual_Discharge', 'May-Oct_Discharge'])

        # 按 year 汇总 Annual_Discharge 和 May-Oct_Discharge  # English: according to
        grouped = df.groupby('year', as_index=False).sum()

        # 如果 Yearly_total 是空的，则初始化  # English: if
        if Yearly_total is None:
            Yearly_total = grouped
        else:
            # 按 year 合并并累加 Annual_Discharge 和 May-Oct_Discharge  # English: according to
