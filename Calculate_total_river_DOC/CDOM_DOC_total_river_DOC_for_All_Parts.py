...
# 初始化列表  # English: Initialization list
total_doc_list = []
total_doc_uncertainty_list = []

# 遍历指定文件  # English: Traversal of specified files
for file in target_files:
    file_path = os.path.join(input_folder, file)
    if os.path.exists(file_path):
        df = pd.read_csv(file_path)

        # 检查列名是否符合预期  # English: Check if the column name meets expectations
...
# 合并并按年聚合  # English: Merge and aggregate by year
...
# 合并两个 DataFrame  # English: Merge two
