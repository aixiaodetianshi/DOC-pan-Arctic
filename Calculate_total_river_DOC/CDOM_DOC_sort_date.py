...
for filename in os.listdir(input_folder):
    ...
    if os.stat(in_file_path).st_size == 0:  # 文件大小为0字节  # English: The file size is
        print(f"文件 {filename} 是空的，创建空文件。")
        open(out_file_path, 'w').close()  # 直接创建一个同名的空文件  # English: Create an empty file with the same name directly
        continue  # 跳过该文件，继续处理下一个  # English: Skip this file

    try:
        # 读取 CSV 文件  # English: Read
        df_DOC = pd.read_csv(in_file_path)

        # 如果 DataFrame 为空，创建空文件  # English: if
        if df_DOC.empty:
            print(f"文件 {filename} 没有数据记录，创建空文件。")
            open(out_file_path, 'w').close()
            continue  # 跳过该文件，继续处理下一个  # English: Skip this file

        # 确保 'date' 列为日期格式  # English: make sure
        df_DOC['date'] = pd.to_datetime(df_DOC['date'], errors='coerce')

        # 按照 'date' 列进行排序  # English: according to
        df_sort_date = df_DOC.sort_values(by='date').reset_index(drop=True)

        # 保存排序后的文件  # English: Save the sorted file
        ...
        open(out_file_path, 'w').close()  # 直接创建一个同名的空文件  # English: Create an empty file with the same name directly
        continue  # 跳过该文件  # English: Skip this file
