...
output_bar_fig = os.path.join(output_dir, "mean_increase_rate_5class_Unit_De_histogram.tif")

# 读取数据  # English: Read data
df = pd.read_csv(input_file)
df['Annual_Increase_Rate_DOC_Area_Unit'] = df['Annual_Increase_Rate_DOC_Area_Unit'] * (-1)
...
# 绘图  # English: Drawing
fig, ax = plt.subplots(figsize=(4/2.54, 2/2.54), dpi=600)  # 大小为 6cm x 4cm  # English: Size is
...
yticks = [1e-5, 1e-7, 1e-9]
yticklabels = [r'$10^{-5}$', r'$10^{-7}$', r'$10^{-9}$']
...
for bar in bars:
    ...
    ax.annotate(f'{height:.2e}',
                xy=(bar.get_x() + bar.get_width() / 2, 1e-9),
                xytext=(0, 5),
