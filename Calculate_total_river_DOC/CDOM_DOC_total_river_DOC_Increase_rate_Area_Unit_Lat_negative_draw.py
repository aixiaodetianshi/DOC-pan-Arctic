...
output_path = r"D:\UZH\2024\20240122 Nutrient and Organic Carbon references\3_river_mouth_DOC\DOC_update_20250203\Total_DOC_increase_rate\Annual_increase_rate_DOC_Unit_De.tif"

# 读取数据  # English: Read data
df = pd.read_csv(file_path)
df['Annual_Increase_Rate_DOC_Area_Unit'] = df['Annual_Increase_Rate_DOC_Area_Unit'] * (-1)  # 单位为 Tg C/yr  # English: The unit is
df = df.dropna(subset=['area_km2', 'Annual_Increase_Rate_DOC_Area_Unit', 'center_lat'])
...
fig, ax = plt.subplots(figsize=(20/2.54, 5/2.54), dpi=600)
...
for i in range(5):
    ...
    ax.scatter(subset['center_lat'], subset['Annual_Increase_Rate_DOC_Area_Unit'],
...
ax.set_xlim(49, 83.5)
ax.set_ylabel('Decrease rate(Tg C yr$^{-1}$ km$^{-2}$)', **font_kwargs)
ax.tick_params(labelsize=6)

# 设置 Y 轴为对数坐标轴，并确保标签为 10 的幂次方  # English: set up
ax.set_yscale('log')
# 设置手动刻度  # English: Setting manual ticks
yticks = [1e-1, 1e-3, 1e-5, 1e-7, 1e-9, 1e-11, 1e-13]
yticklabels = [r'$10^{-1}$', r'$10^{-3}$', r'$10^{-5}$', r'$10^{-7}$', r'$10^{-9}$', r'$10^{-11}$', r'$10^{-13}$']
