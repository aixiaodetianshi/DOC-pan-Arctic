...
# 设置输入和输出文件夹路径  # English: Set input and output folder paths
...
# 遍历两个输入文件夹中所有的文件名  # English: Iterate through all file names in two input folders
all_filenames = set(os.listdir(in_folder_greater_50)) | set(os.listdir(in_folder_greater_equal_0))

for filename in all_filenames:
    # 初始化空DataFrame  # English: Initialize empty
    combined_df = pd.DataFrame()

    # 读取第一个文件夹中的数据  # English: Read data in the first folder
    ...
    # 读取第二个文件夹中的数据  # English: Read data in the second folder
    ...
    # 如果DataFrame不为空，按时间排序并保存  # English: if
