...
output_area_fig = os.path.join(output_dir, "Fig1_river_number_piechart.tif")

# 分组定义  # English: Grouping definition
river_Name = ["Ob", "Yenisey", "Lena", "Kolyma", "Yukon", "Mackenzie"]
river_number = [2360, 125, 643, 631, 331, 6466]
...
for name, num, pct in zip(river_Name, river_number, river_percent):
    print(f"{name}: {num} 条河流，占比 {pct:.2f}%")
...
# 标题  # English: title
ax1.set_title("river number(%)", fontsize=6, pad=-160)
