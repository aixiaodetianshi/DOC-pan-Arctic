...
# 设置河流名称和输入路径  # English: Set the river name and input path
river = "All"
...
# 读取CSV文件，提取需要的列  # English: Read
df = pd.read_csv(input_file_path, usecols=['year_month', 'Total_DOC', 'Total_DOC_uncertainty'])

# 将 year_month 转换为 datetime 格式  # English: Will
# df['year_month'] = pd.to_datetime(df['year_month'], format='%Y-%m')
df['year_month'] = pd.to_datetime(df['year_month'], errors='coerce')

# 只提取每年5月的数据（即月份为5）  # English: Extract only every year
...
# 只提取每年6月的数据（即月份为6）  # English: Extract only every year
...
# 只提取每年7月的数据（即月份为7）  # English: Extract only every year
...
# 只提取每年8月的数据（即月份为8）  # English: Extract only every year
...
# 只提取每年9月的数据（即月份为5）  # English: Extract only every year
...
# 只提取每年10月的数据（即月份为6）  # English: Extract only every year
