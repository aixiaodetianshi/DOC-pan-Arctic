...
output_path = r"D:\UZH\2024\20240122 Nutrient and Organic Carbon references\DOC\DOC_update_20250203\Total_DOC_increase_rate\ARCADE_MERIT_pan_Arctic_Increase_rate_DOC_Area_1Class.tif"
...
# 仅保留第一级分类（1–10 km²）  # English: Only the first-level classification is retained
df = df[(df['area_km2'] >= 1) & (df['area_km2'] < 10)]
...
ax.set_xscale('log')
ax.set_xlim(1, 10)  # 只显示第一级范围  # English: Show only the first level range
ax.set_ylim(bottom=0)  # 可选：设置 Y 轴从 0 开始  # English: Optional
