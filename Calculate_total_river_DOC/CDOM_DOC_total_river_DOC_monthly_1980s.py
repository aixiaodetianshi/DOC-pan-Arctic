...
# 添加月份图例，显示在图左侧，竖直排列（每个月一行）  # English: Add a month legend
legend2 = ax.legend(
    month_handles,
    month_labels,
    loc='center left',  # 图左边垂直居中  # English: The left side of the picture is vertically centered
    bbox_to_anchor=(-0.025, 0.5),  # 调整更靠左位置，可根据实际图形微调  # English: Adjust more to the left
    ncol=1,  # 一列 = 竖直显示  # English: A column
    frameon=False,
    fontsize=6,
    handletextpad=0.1,
    handlelength=1.0,  # 控制图标长度（默认 2.0）  # English: Control icon length
    labelspacing=0.4
)

# 添加第一个图例后必须手动添加回去，否则会被第二个覆盖  # English: After adding the first legend, you must add it back manually
