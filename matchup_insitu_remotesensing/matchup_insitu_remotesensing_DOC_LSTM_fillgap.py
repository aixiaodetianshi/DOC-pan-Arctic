...
look_back = 100  # 增加look_back的值以捕获更多的时间依赖性  # English: Increase
X, Y = create_dataset(scaled_data, look_back)

# 调整X的形状以适应LSTM的输入要求  # English: Adjustment
X = np.reshape(X, (X.shape[0], X.shape[1], 1))

# 创建LSTM模型  # English: create
model = Sequential()
...
# 使用GPU进行训练  # English: use
with tf.device('/GPU:0'):
    model.fit(X, Y, epochs=50, batch_size=32, verbose=2)  # 增加epochs和batch_size以加强模型学习  # English: Increase

# 保存模型  # English: Save the model
model.save(model_file)

# 创建输入数据用于预测  # English: Create input data for prediction
def predict_fill_gaps(data, model, look_back):
    ...
    for i in range(len(data)):
        if i >= look_back:
            ...
            data[i] = pred  # 更新输入数据  # English: Update input data
        else:
            predictions.append(data[i, 0])
    return np.array(predictions)

# 填补缺失值  # English: Fill in missing values
scaled_data = scaled_data.reshape(-1, 1)
predicted = predict_fill_gaps(scaled_data, model, look_back)

# 反归一化以得到原始尺度的值  # English: Reverse normalization to get the value of the original scale
predicted_full = scaler.inverse_transform(predicted.reshape(-1, 1))

# 只填补原缺失值  # English: Only fill in the original missing value
df['DOC_filled'] = df['DOC'].copy()
df.loc[missing_values_mask, 'DOC_filled'] = predicted_full[missing_values_mask.values]

# 限制 DOC_filled 的值在原始 DOC 的最大值和最小值之间  # English: limit
...
# 绘制原始和填补后的DOC时间序列  # English: Draw original and filled
