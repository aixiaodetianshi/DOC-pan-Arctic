...
#
# input_shp 是原始 shapefile 文件路径。  # English: It's the original
# comid_folder 是包含文件名等于 COMID 的文件夹路径。  # English: is the file name equal to
# 提取 COMID 列表:  # English: extract
#
# 从文件夹中提取所有文件名，并将其与 shapefile 的 COMID 属性进行匹配。  # English: Extract all file names from folder
# 筛选 GeoDataFrame:  # English: filter
#
# 使用 isin() 方法筛选 shapefile 中 COMID 与文件夹文件名匹配的多边形。  # English: use
# 确保输出文件夹存在:  # English: Make sure the output folder exists
#
# 使用 os.makedirs() 创建保存结果文件的文件夹路径。  # English: use
# 保存为新 shapefile:  # English: Save as new
#
# 使用 to_file() 方法保存提取后的 shapefile 为 Ob.shp。  # English: use
# 注意事项  # English: Things to note
# 确保 COMID 列的数据类型与文件名一致（如需要，将其转换为字符串）。  # English: make sure
# 输出文件夹 Ob 需存在或可由程序自动创建。  # English: Output folder
# 如果 COMID 列的命名有变化，请修改代码中对 COMID 的引用。  # English: if
