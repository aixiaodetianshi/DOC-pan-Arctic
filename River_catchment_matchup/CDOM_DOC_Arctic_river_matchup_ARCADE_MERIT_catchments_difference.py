# 使用 geopandas 来比较两个 shapefile，  # English: use
# 根据属性 gid 找出 ARCADE_exclude_Greenland_Catchment  # English: According to attributes
# 中不在 ARCADE_Ob_Mackenzie 中的 58 个要素，并保存为一个新的 shapefile。  # English: Not here
...
# 加载两个 shapefile  # English: Load two
catchment_gdf = gpd.read_file(catchment_file)
ob_mackenzie_gdf = gpd.read_file(ob_mackenzie_file)

# 提取不在 ARCADE_Ob_Mackenzie 中的 gid  # English: Extraction is not there
remaining_gdf = catchment_gdf[~catchment_gdf['gid'].isin(ob_mackenzie_gdf['gid'])]

# 检查结果数量是否正确  # English: Check if the number of results is correct
print(f"剩余要素数量: {len(remaining_gdf)}")  # 应为 58  # English: Should be

# 保存剩余要素为新的 shapefile  # English: Save remaining features as new
