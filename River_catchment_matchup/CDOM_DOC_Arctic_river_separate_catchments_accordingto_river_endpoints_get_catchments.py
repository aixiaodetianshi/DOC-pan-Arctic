...
# 依据上面的得到的河流入海口的流域内河流reach数据，每条河流入海口对应一个excel文件，  # English: The river flows in the basin of the river in the mouth of the sea obtained above
# 文件内有一列是河流reach的识别编号COMID，  # English: There is a column in the file that is a river
# 读取文件夹D:\UZH\2024\20240122 Nutrient and Organic Carbon references\6large_rivers_watersheds\6River_Parts_river_reachs\Ob_COMID内的每条河流excel文件，  # English: Read folder
# 得到该条河流的COMID数据，读取shp文件D:\UZH\2024\20240122 Nutrient and Organic Carbon references\6large_rivers_watersheds\Arctic_catchments\Ob_catchments.shp  # English: Get the river's
# 内的所对应的COMID流域，将这些流域数据以相同的文件名，shp文件格式，  # English: The corresponding one within
# 写入文件夹D:\UZH\2024\20240122 Nutrient and Organic Carbon references\6large_rivers_watersheds\6River_Parts_watrersheds\Ob中，  # English: Write to folder
...
# 读取流域的 shapefile 数据  # English: Read the basin
catchments_gdf = gpd.read_file(shp_file)

# 检查 shapefile 是否包含 COMID 和 unitarea 列  # English: examine
...
# 遍历 Excel 文件夹中的每个文件  # English: Traversal
for excel_file in os.listdir(excel_folder):
    if excel_file.endswith(".xlsx"):

        # 读取 Excel 文件中的 COMID 数据  # English: Read
