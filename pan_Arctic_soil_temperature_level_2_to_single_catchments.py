...
# 输入输出路径
input_folder = r'D:\UZH\2025\NPP\pan_Arctic_soil_temperature_level_2_multi_catchments'
output_folder = os.path.join(input_folder, 'catchments')
...
for filename in os.listdir(input_folder):
    if filename.lower().endswith('.csv'):
        ...
        for i in range(num_catchments):
            ...
            # 提取 date 和 soil_temperature_level_2 列
            catchment_df = sub_df[['date', 'soil_temperature_level_2']].copy()

            # 转换单位：K -> °C
            catchment_df['soil_temperature_level_2'] = catchment_df['soil_temperature_level_2'] - 273.15

            # 获取 gid 作为文件名
            gid = sub_df['gid'].iloc[0]
            output_filename = f'{gid}_soil_temperature_level_2.csv'
