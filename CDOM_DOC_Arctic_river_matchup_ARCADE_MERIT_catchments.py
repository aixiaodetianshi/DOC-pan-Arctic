# 计算最近距离：
# 使用 nearest() 方法计算每个 ARCADE 多边形到 MERIT Hydro 数据集中最近多边形的距离。
# 计算空间重叠：
# 使用 geopandas.overlay() 找到所有交集并计算交集面积。
# 按照每个 ARCADE 多边形找到重叠面积最大的 MERIT Hydro 多边形。
# 综合匹配：
# 如果一个 ARCADE 多边形有多个重叠候选，则优先选择重叠率最高的；如果没有重叠，则选取最近距离的多边形。
# 添加属性：
# 将选定的 COMID 添加为 ARCADE 多边形的新属性。
