...
input_folder = r'D:\UZH\2025\NPP\pan_Arctic_temperature_2m\batch'
output_folder = r'D:\UZH\2025\NPP\pan_Arctic_temperature_2m'
...
for filename in os.listdir(input_folder):
    if filename.lower().endswith('.csv'):
        ...
        for i in range(num_catchments):
            ...
            # 提取 date 和 temperature_2m 列  # English: extract
            catchment_df = sub_df[['date', 'temperature_2m']].copy()

            # 转换单位：K -> °C  # English: Convert Units
            catchment_df['temperature_2m'] = catchment_df['temperature_2m'] - 273.15

            # 获取 gid 作为文件名  # English: Get
            gid = sub_df['gid'].iloc[0]
            output_filename = f'{gid}_temperature_2m.csv'
