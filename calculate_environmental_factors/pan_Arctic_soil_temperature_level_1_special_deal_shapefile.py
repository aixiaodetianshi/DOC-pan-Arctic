# 从温度数据文件夹中提取已有的 gid 值；  # English: Extract existing ones from the temperature data folder
# 
# 加载 .shp 文件；  # English: load
# 
# 筛选出 不在已有温度数据中的流域；  # English: Filter out
# 
# 输出筛选结果为新的 Shapefile。  # English: Output filter result is new
...
# 1. 加载流域 shapefile  # English: Loading basin
gdf = gpd.read_file(shapefile_path)

# 2. 获取已存在温度文件的 gid 列表  # English: Get the existing temperature file
existing_gids = set()
...
# 3. 筛选不在已有 GID 中的流域  # English: Filter is not available
gdf['gid'] = gdf['gid'].astype(int)
gdf_filtered = gdf[~gdf['gid'].isin(existing_gids)]

# 4. 保存结果  # English: Save the results
