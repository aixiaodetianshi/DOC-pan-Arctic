# 使用系统偏差校准过得Landsat7 来验证Landsat 5  # English: Use system deviation calibration
import os
import pandas as pd
...
# 创建索引以加速匹配  # English: Create indexes to speed up matches
df_landsat5.set_index("COMID", inplace=True)
results = []

# 按照±1天范围匹配相同 COMID 的数据  # English: according to
print("开始匹配数据（±1天）...")
for _, row in tqdm(df_landsat7.iterrows(), total=len(df_landsat7)):
    ...
    if isinstance(sub_df, pd.Series):  # 若只有一条记录则转为DataFrame  # English: If there is only one record, it will be converted to
        sub_df = pd.DataFrame([sub_df])

    # 筛选±1天的数据  # English: filter
...
# 仅保留 DOC 值都小于等于 30 的记录，剔除异常高值  # English: Keep only
...
# 评估指标计算  # English: Evaluation indicator calculation
