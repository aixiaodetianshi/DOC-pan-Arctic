...
# 解决 Hugging Face 缓存问题  # English: solve
os.environ["HF_HUB_DISABLE_SYMLINKS_WARNING"] = "1"

# 关闭 TensorFlow OneDNN 以减少无关警告  # English: closure
os.environ["TF_ENABLE_ONEDNN_OPTS"] = "0"

# 初始化 OpenAI GPT-4  # English: initialization
os.environ["OPENAI_API_KEY"] = "sk-...svEA"  # 这里需要替换为你的 API Key  # English: It needs to be replaced by yours

# 加载 NLP 处理模型  # English: load
nlp = spacy.load("en_core_web_sm")
summarizer = pipeline("summarization", model="facebook/bart-large-cnn")
llm = ChatOpenAI(model_name="gpt-4", temperature=0.5)

# 读取 PDF 文件并提取文本  # English: Read
def extract_text_from_pdf(pdf_path):
    with open(pdf_path, "rb") as file:
        reader = PyPDF2.PdfReader(file)
        text = "\n".join([page.extract_text() for page in reader.pages if page.extract_text()])
    return text[:5000]  # 限制 PDF 长度，避免超出 BART 处理范围  # English: limit

# 提取关键词  # English: Extract keywords
def extract_keywords(text, num_keywords=5):
    doc = nlp(text)
    keywords = [token.text for token in doc if token.is_alpha and not token.is_stop]
    return list(set(keywords[:num_keywords]))

# 生成摘要（确保不会超出 BART 的 token 限制）  # English: Generate summary
...
# 生成综述  # English: Generate a review
...
# 处理所有 PDF 文件  # English: Process all
...
# 生成文献综述  # English: Generative literature review
final_review = generate_review(summaries)

# 输出到 Markdown 文件  # English: Output to
