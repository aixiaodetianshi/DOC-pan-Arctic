# 利用结合的 HLSL30 和 HLSS30 进行验证 Landsat 7  # English: Utilize combined
...
def load_and_merge_data(folder, hls30_file, landsat7_file, output_file):
    ...
    # 遍历每个HLS30日期，寻找±3天范围内的Landsat7数据  # English: Iterate through each
    for _, row in df_hls30.iterrows():
        date = row['date']
        doc_hls = row['DOC_HLS30']

        # 查找Landsat7数据中，日期在±3天之内的记录  # English: Find
        # 查找 Landsat7 数据中，日期在 ±3 天之内的记录  # English: Find
        mask = (df_landsat7['date'] >= date - timedelta(days=10)) & \
               (df_landsat7['date'] <= date + timedelta(days=10))
        matching = df_landsat7.loc[mask]

        if not matching.empty:
            doc_landsat7_mean = matching['DOC_Landsat7'].mean()
            merged_rows.append({'date': date, 'DOC_HLS30': doc_hls, 'DOC_Landsat7': doc_landsat7_mean})
...
base_dir = r'D:\UZH\2024\20240122 Nutrient and Organic Carbon references\1_Linear_Regression_DOC_CDOM\4_Cross_Validate_Landsat_5_Landsat7_HLSL30_HLSS30_DOC\HLSL30_HLSS30_combine_calibrate_Landsat7\Yenisey'
hls30_file = 'Cross_validate_Combine_HLSL30_HLSS30_Yenisey_DOC.csv'
landsat7_file = 'Cross_validate_Landsat7_Yenisey_DOC.csv'
input_filename = 'Cross_validate_HLS30_Landsat7_Yenisey_DOC.csv'

summary_filename = 'Segment_Model_Summary_Linear.csv'
calibrated_output_filename = 'Cross_validate_HLS30_Landsat7_Yenisey_cor_DOC.csv'
...
df_clean = df.copy()  # 实际未剔除残差异常值  # English: The residual difference constant value has not been removed
...
q33 = df_clean['DOC_Landsat7_corrected'].quantile(0.2)
q66 = df_clean['DOC_Landsat7_corrected'].quantile(0.5)
