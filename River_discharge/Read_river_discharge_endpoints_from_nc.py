...
# 读取河流编号 Excel 文件  # English: Read the river number
reach_numbers_df = pd.read_excel(reach_file_path)
reach_numbers = reach_numbers_df['COMID'].astype(int).tolist()  # 假设 Excel 中编号列名为 'COMID'  # English: Assumptions

# 读取 NetCDF 文件  # English: Read
file_obj = nc.Dataset(nc_file_path)

# 获取时间和流量变量  # English: Get time and traffic variables
days = np.arange(0, 12784)
dates = [start_date + datetime.timedelta(days=int(day)) for day in days]  # 计算每一天的日期  # English: Calculate the dates of each day
discharge = file_obj.variables['discharge']
reach_data = file_obj.variables['reach'][:].astype(int)  # 将 reach_data 转换为整型数组  # English: Will

# 循环读取每个 reach 编号的数据  # English: Looping each
for reach_no in reach_numbers:
    try:
        # 找到对应的 reach 编号的索引  # English: Find the corresponding one
        reach_index = np.where(reach_data == reach_no)[0][0]
        discharge_v = discharge[0:12784, reach_index]  # 获取该 reach 编号的流量数据  # English: Get this

        # 创建 DataFrame 并保存为 Excel 文件  # English: create
        df = pd.DataFrame({
            'time': dates,
            'discharge': discharge_v
        })

        # 保存文件，文件名格式为 reach_no.xlsx  # English: Save the file
