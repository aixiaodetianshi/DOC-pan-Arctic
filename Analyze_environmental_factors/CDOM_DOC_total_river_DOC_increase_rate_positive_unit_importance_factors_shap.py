...
target_col = 'Annual_Increase_Rate_DOC_Area_Unit'
df = df.dropna(subset=[target_col])
df = df[df[target_col] > 0]  # 只保留增长率为正的记录  # English: Only records with positive growth rates are retained

drop_cols = ['area_km2', 'perimeter', 'Average_Total_DOC', 'Average_Total_DOC_1', 'Average_Total_DOC_Area_Unit', 'Annual_Increase_Rate_1', 'Average_Total_DOC_Uncertainty', 'center_lon', 'COMID', 'Annual_Increase_Rate', 'Intercept', 'R_Value', 'P_Value', 'Std_Err', 'Num_same_COMID', ]
...
result_df = result_df.sort_values(by='RandomForest_Importance', ascending=False)  # 便于横向 barplot 显示  # English: Easy to horizontally
...
output_path = r"D:\UZH\2024\20240122 Nutrient and Organic Carbon references\3_river_mouth_DOC\DOC_update_20250203\Total_DOC_increase_rate\Unit_Top20_Feature_Importance_positive.tif"
